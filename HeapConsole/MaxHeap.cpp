...
unsigned int MaxHeap::GetLength() {
	return length;
}
...
void MaxHeap::InsertNode(HeapNode& newNode) {
	length++;
	if (!root) {
		root = &newNode;
		carry = 1;
...
void MaxHeap::Swim(HeapNode& target) {
	if (root == &target)
		return;
...
void MaxHeap::Sink(HeapNode& target) {
	HeapNode* current = &target;
...
void MaxHeap::Push(int key, string& data) {
...
// for debug
bool MaxHeap::HealthCheck(HeapNode* current, int& currentHeight, int& diff) {
